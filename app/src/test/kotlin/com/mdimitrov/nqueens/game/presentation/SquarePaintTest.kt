...
class SquarePaintTest {
    ...
    fun `the queen is drawn only where she stands, and reddened when she is attacked`() {
        assertNull(queenTint(CellStatus.EMPTY, colors))
        assertEquals(colors.queen, queenTint(CellStatus.PIECE, colors))
        assertEquals(colors.conflict, queenTint(CellStatus.PIECE_CONFLICT, colors))
    }
