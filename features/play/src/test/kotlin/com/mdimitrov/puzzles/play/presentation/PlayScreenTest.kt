...
import com.mdimitrov.puzzles.play.presentation.sound.Sounds
import com.mdimitrov.puzzles.play.presentation.win.CELEBRATION_TAG
...
class PlayScreenTest {
    ...
    private val played = mutableListOf<PlaySound>()
    private val haptics =
    ...
    fun `on the narrowest phone the largest board keeps the floor and the screen scrolls`() {
        boardWith(size = Queens.sizes.last)

        // 320 dp across is where a twelve-board first meets the floor: below it the squares would
        // shrink rather than the screen scroll, which is the trade the floor exists to refuse.
        compose.onNodeWithContentDescription(square(1)).assertHeightIsAtLeast(MIN_SQUARE)
        compose.onNodeWithContentDescription(square(Queens.sizes.last))
            .performScrollTo()
    ...
    fun `the largest board stays playable on a wide screen`() {
        ...
        compose.onNodeWithContentDescription("Row ${Queens.sizes.last}, column ${Queens.sizes.last}, empty")
            .assertIsDisplayed()
            .assertHeightIsAtLeast(MIN_SQUARE)
    }
    ...
    fun `on a phone the largest board is whole, hittable, and the details are under it`() {
        ...
        assertHittable(Queens.sizes.last)
        val corner = compose.onNodeWithContentDescription(square(Queens.sizes.last)).getUnclippedBoundsInRoot()
    ...
    fun `on a screen wider than it is tall the details sit beside the board`() {
        ...
        compose.onNodeWithText("${Queens.sizes.last} × ${Queens.sizes.last} · Queens").assertExists()
    }
    ...
    fun `in a split screen window the strip still has room to say what is wrong`() {
        boardWith(Cell(0, 0), Cell(0, 3), size = Queens.sizes.last)
    ...
    fun `the screen speaks the words the puzzle gives it`() {
        val other =
            Queens.copy(
                name = R.string.play_reset,
                text =
                    Queens.text.copy(
                        counter = R.string.play_back,
                        cell = android.R.string.copy,
                        cellUnderAttack = android.R.string.cut,
                        idle = android.R.string.paste,
                    ),
            )
        compose.setContent {
            PuzzleTheme {
        ...
        compose.onAllNodesWithContentDescription("Cut").assertCountEquals(2)
        compose.onNodeWithContentDescription("Copy").assertExists()
    }

    @Test
    fun `the strip speaks the puzzle's words when nothing is wrong`() {
        val other = Queens.copy(text = Queens.text.copy(idle = android.R.string.paste))
        compose.setContent {
            PuzzleTheme {
                PlayContent(
                    state = PlayUiState(uiState(arrayOf(Cell(0, 0)), BOARD_SIZE).board, other, 0),
                    actions =
                        PlayActions(
                            onTap = { },
                            onReset = { },
    ...
    fun `a board finished faster than before says by how much`() {
        ...
        compose.setContent {
            PuzzleTheme {
                PlayContent(
                    state = PlayUiState(solved, Queens, WINNING_TIME, previousBestSeconds = WINNING_TIME + 12),
                    actions =
                        PlayActions(
                            onTap = { },
                            onReset = { },
    ...
    fun `the win card leads on to the scores`() {
        ...
        compose.setContent {
            PuzzleTheme {
                PlayContent(
                    state = PlayUiState(solved, Queens, WINNING_TIME),
                    actions = PlayActions(onTap = {}, onReset = {}, onBack = {}, onScores = { asked = true }),
                )
            }
        }
    ...
    fun `placing a queen is heard, and taking her back sounds different`() {
        ...
        assertEquals(listOf(PlaySound.PLACE, PlaySound.REMOVE), played)
    }
    ...
    fun `a move that puts a queen under attack sounds once, and easing the trouble is quiet`() {
        ...
        compose.waitForIdle()
        assertEquals(listOf(PlaySound.CONFLICT), played, "the move that created the attack")
        ...
        compose.waitForIdle()
        assertEquals(listOf(PlaySound.CONFLICT), played, "an attack resolved is not an attack made")
    }
    ...
    fun `the win is announced once, however often the screen comes back`() {
        ...
        assertEquals(listOf(PlaySound.WIN), played, "the win sounded again on the way back")
    ...
    fun `a solved board is heard as well as felt`() {
        ...
        assertEquals(listOf(PlaySound.WIN), played)
    }
    ...
    private fun uiState(
        ...
        board = snapshotOf(GameState(size = size, pieces = queens.toSet()), NQueensLines, target = size),
        puzzle = Queens,
        elapsedSeconds = 0,
    )
    ...
    private fun solvedBoard(
        ...
        compose.setContent {
            CompositionLocalProvider(
                ...
                PuzzleTheme {
                    PlayContent(
                        state = PlayUiState(solved, Queens, elapsed, previousBest),
                        actions = PlayActions(onTap = {}, onReset = {}, onBack = {}, onScores = {}),
                    )
                }
            }
