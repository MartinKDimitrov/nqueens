...
- **What the board answers with.** A piece set down and one lifted are felt and heard, each with
  ...
  trouble without ending it is silent. The sounds are generated by `design/sounds.py` rather than
  recorded, and the game is quiet while the phone is.
...
it would pass every one of them.
...
once, against Robolectric's own pool: that a sound reaches it, and that a phone on silent
gets none.
