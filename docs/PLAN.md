...
**Step 8.2 — Sound.** *(not built)*
- Four sounds, generated by a script that is checked in beside them: a wooden clack when a piece
  is set down, a lighter one when it is lifted, a dull beat when a move puts a piece under attack,
  and a chord when the board is solved.
- The conflict sound belongs to the *transition* — the move that creates a new attack — not to
  the state, or it repeats on every move afterwards.
